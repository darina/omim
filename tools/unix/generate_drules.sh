...
function BuildDrawingRules() {
  ...
  python2 "$OMIM_PATH/tools/kothic/src/libkomwm.py" --txt \
...
python2 "$OMIM_PATH/tools/python/transit/transit_colors_export.py" \
  "$DATA_PATH/colors.txt" > /dev/null

echo "Merging default and vehicle styles"
python2 "$OMIM_PATH/tools/python/stylesheet/drules_merge.py" \
